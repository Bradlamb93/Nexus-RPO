...
const FCCLogo = ({size=32,textColor="#ffffff",showText=false,textSize=17}) => (
  <div style={{display:"flex",alignItems:"center",gap:9}}>
    <svg width={size} height={size} viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* Left link arc */}
      <path d="M22 10 C15.5 10 10 14.5 10 20 C10 25.5 15.5 30 22 30" stroke="white" strokeWidth="3.5" fill="none" strokeLinecap="round"/>
      {/* Right link arc */}
      <path d="M18 10 C24.5 10 30 14.5 30 20 C30 25.5 24.5 30 18 30" stroke="white" strokeWidth="3.5" fill="none" strokeLinecap="round"/>
      {/* Centre dot */}
      <circle cx="20" cy="20" r="2.5" fill="white"/>
    </svg>
    {showText && (
      <span style={{fontFamily:"Instrument Serif,serif",fontSize:textSize,color:textColor,letterSpacing:"-0.01em",lineHeight:1}}>
...
// Rate cards — agency rates (what FCC pays agencies) and client rates (what FCC charges care homes)
...
// Neutral vendor margin — what FCC charges care homes on top of agency rates
const INIT_MARGIN_CONFIG = {
  ...
  notes: "Applied to all hours billed through the platform. Agencies see their base rate; care homes see the client rate inclusive of the FCC margin.",
};
...
const INIT_USERS = [
  // Admin users
  {id:"u1",name:"Rachel Obi",email:"r.obi@firstchoiceconnect.co.uk",role:"admin",org:"First Choice Connect",status:"active",lastLogin:"2026-03-10",superAdmin:true,perms:defaultPerms("admin"),createdAt:"2024-01-01"},
  {id:"u2",name:"Tom Bright",email:"t.bright@firstchoiceconnect.co.uk",role:"admin",org:"First Choice Connect",status:"active",lastLogin:"2026-03-09",superAdmin:false,perms:{...defaultPerms("admin"),ratecards:false,users:false},createdAt:"2024-06-15"},
  // Care home users
...
const INIT_COMPLIANCE_REQS = [
  // Global — set by First Choice Connect admin, apply to all sites
  {id:"cr1",name:"DBS Enhanced Certificate",type:"document",category:"safeguarding",appliesToRoles:["RGN","HCA","RMN","Senior Carer"],mandatory:true,expiryMonths:36,scope:"global",addedBy:"FCC Admin",careHome:null,createdAt:"2024-01-01",active:true,notes:"Must be on the Update Service or renewed within 3 years"},
  {id:"cr2",name:"Mandatory Training Certificate",type:"training",category:"training",appliesToRoles:["RGN","HCA","RMN","Senior Carer"],mandatory:true,expiryMonths:12,scope:"global",addedBy:"FCC Admin",careHome:null,createdAt:"2024-01-01",active:true,notes:"Covers: safeguarding, fire safety, infection control, moving & handling"},
  {id:"cr3",name:"NMC/PIN Registration",type:"registration",category:"registration",appliesToRoles:["RGN","RMN"],mandatory:true,expiryMonths:12,scope:"global",addedBy:"FCC Admin",careHome:null,createdAt:"2024-01-01",active:true,notes:"Must be active and without conditions. Checked against NMC register."},
  {id:"cr4",name:"COVID-19 Vaccination Record",type:"document",category:"health",appliesToRoles:["RGN","HCA","RMN","Senior Carer"],mandatory:false,expiryMonths:null,scope:"global",addedBy:"FCC Admin",careHome:null,createdAt:"2024-01-01",active:true,notes:"Recommended but not mandated. Record primary course + boosters."},
  {id:"cr5",name:"Right to Work Evidence",type:"document",category:"legal",appliesToRoles:["RGN","HCA","RMN","Senior Carer"],mandatory:true,expiryMonths:null,scope:"global",addedBy:"FCC Admin",careHome:null,createdAt:"2024-01-01",active:true,notes:"Passport, visa, or share code verification"},
...
const Alert = ({type="info",children}) => {
  const cfg = {info:{bg:T.blueBg,border:T.blue,color:T.blue},warn:{bg:T.yellowBg,border:T.yellow,color:T.yellow},error:{bg:T.redBg,border:T.red,color:T.red},success:{bg:T.greenBg,border:T.green,color:T.green}};
  const c = cfg[type];
...
const exportHTML = (title, subtitle, tableHTML) => {
  ...
  <div class="header">
    ...
    <div class="meta"><div>Generated: ${new Date().toLocaleDateString("en-GB",{day:"2-digit",month:"short",year:"numeric"})}</div><div>First Choice Connect Ltd</div></div>
  </div>
  <h1>${title}</h1><p class="subtitle">${subtitle||""}</p>
  ${tableHTML}
  <div class="footer"><span>First Choice Connect — Confidential</span><span>Page 1</span></div>
  </body></html>`;
...
const AuthScreen = ({onAuth}) => {
  ...
  const handle = () => {
    ...
    setError("");
    onAuth({email,name:name||"Admin User",role,org:org||"First Choice Connect"});
  };
  ...
  return (
    <div style={{minHeight:"100vh",display:"grid",gridTemplateColumns:"1fr 1fr",fontFamily:"Syne,sans-serif"}}>
      ...
      <div style={{background:`linear-gradient(145deg,${T.navy} 0%,#1a3060 100%)`,display:"flex",flexDirection:"column",justifyContent:"space-between",padding:"48px"}}>
        ...
        <p style={{color:"rgba(255,255,255,0.2)",fontSize:11}}>© 2026 First Choice Connect Ltd. All rights reserved.</p>
      </div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"center",padding:"32px 48px",background:"#fafbfd",overflowY:"auto"}}>
        <div style={{width:"100%",maxWidth:400}}>
          ...
          <p style={{fontSize:13,color:T.muted,marginBottom:20}}>{mode==="login"?"Sign in to your First Choice Connect portal":"Set up your First Choice Connect account"}</p>
          ...
          {mode==="login" && (
            <div style={{background:T.amberBg,border:`1.5px solid ${T.amber}55`,borderRadius:10,padding:"12px 14px",marginBottom:20}}>
              ...
              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                {[
                  {r:"admin",       l:"FCC Admin",      name:"Rachel Obi"},
                  ...
                  <button key={x.r}
                    onClick={()=>onAuth({email:"demo@example.com",name:x.name,role:x.r,org:x.r==="admin"?"First Choice Connect":x.r==="clientadmin"?"Sunrise Healthcare Group":x.r==="carehome"?"Sunrise Care":x.r==="agency"?"First Choice Nursing":"Bank Staff"})}
...
const INIT_NOTIFICATIONS = {
  ...
  agency:[
    ...
    {id:"n17",type:"rate_uplift",     title:"Rate Request Approved",         body:"Your HCA rate uplift request (£1/hr) was approved by FCC. Effective Mar 2026.",time:"5d ago", read:true,  action:"ratecards"},
  ...
  return (
    <div style={{width:224,minHeight:"100vh",background:T.navy,display:"flex",flexDirection:"column",position:"sticky",top:0,flexShrink:0}}>
      ...
      <div style={{padding:"10px 10px 16px",borderTop:`1px solid ${T.navyBorder}`}}>
        <div style={{fontSize:10,color:"rgba(255,255,255,0.2)",textAlign:"center",marginBottom:8}}>v3.0.0 — Demo Mode</div>
...
const ComplianceTracker = ({complianceReqs,setComplianceReqs,navigate}) => {
  ...
  return (
    <Page title="Compliance" sub="Track worker credentials and manage platform-wide compliance requirements" icon="🛡"
      ...
      {/* Modals */}
      {showForm&&<RequirementFormModal onSave={addReq} onClose={()=>setShowForm(false)} addedBy="FCC Admin" careHome={null}/>}
      {editing&&<RequirementFormModal initial={editing} onSave={saveEdit} onClose={()=>setEditing(null)} addedBy="FCC Admin" careHome={editing.careHome}/>}
...
const CareHomeCompliance = ({complianceReqs,setComplianceReqs,user}) => {
  ...
  return (
    <Page title="Compliance Requirements" sub={`Managing compliance for ${mySite}`} icon="🛡"
      ...
      {/* Summary */}
      <Grid cols={3}>
        <Stat label="Platform Requirements" value={globalReqs.length} sub="Set by First Choice Connect — apply to all sites" accent/>
      ...
      {tab==="overview"&&(
        <>
          <div style={{background:"#f0f7ff",border:`1px solid ${T.blue}44`,borderRadius:12,padding:"12px 16px",marginBottom:16,fontSize:12,color:T.muted}}>
            <strong style={{color:T.blue}}>ℹ️ How this works:</strong> All workers placed at {mySite} must meet both the platform-wide requirements (set by First Choice Connect) and any requirements you've added for your site. Contact your First Choice Connect account manager to query platform requirements.
          </div>
    ...
    <Modal title={isNew ? `Add ${r.type==="agency"?"Agency":"Client"} Rate Card` : `Edit Rate — ${r.role} (${r.band})`} onClose={onClose}>
      ...
      <div style={{background:"#f8fafc",borderRadius:8,padding:"12px 14px",margin:"10px 0 14px"}}>
        <div style={{fontSize:11,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:"0.07em",marginBottom:10}}>
          {r.type==="agency"?"Pay Rate (£/hr — what FCC pays agency)":"Charge Rate (£/hr — what FCC bills client)"}
        </div>
...
const RateCards = ({rateCards, setRateCards}) => {
  ...
  return (
    <Page title="Rate Cards" sub="Set pay rates per agency and charge rates per client" icon="💷">
      ...
      <Alert type="info">Rate changes apply to all new shifts. Existing confirmed shifts are unaffected. Agency rates = what FCC pays. Client rates = what care homes are billed. The difference forms FCC's margin.</Alert>
        ...
        <>
          ...
          {agencyRates.length === 0
            ? <Card style={{padding:32,textAlign:"center"}}>
                ...
                <div style={{color:T.muted,fontSize:13,marginBottom:16}}>Add rate cards to define what FCC pays this agency per role.</div>
            ...
            : <Card>
                <div style={{padding:"12px 18px",background:"#f0fdf4",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:8}}>
                  ...
                  <span style={{fontSize:11,color:T.muted}}>— what FCC pays this agency per hour billed</span>
                </div>
          ...
          {clientRates.length === 0
            ...
            : <Card>
                <div style={{padding:"12px 18px",background:"#eff6ff",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:8}}>
                  ...
                  <span style={{fontSize:11,color:T.muted}}>— what FCC bills this care home per hour</span>
                </div>
...
const InvoiceManager = ({invoices=INVOICES, setInvoices, timesheets=[]}) => {
  ...
  const invExports = [
    ...
    {icon:"🖨️",label:"Invoice Ledger — PDF",desc:"Printable HTML report",fn:()=>exportHTML("Invoice Ledger","First Choice Connect — All Invoices",
...
const Analytics = ({budgets}) => {
  const exports = [
    ...
    {icon:"🖨️",label:"Full Analytics Report — PDF",desc:"Printable HTML report",fn:()=>exportHTML("FCC Analytics Report","January–March 2026",
...
const RequestShift = ({user, navigate}) => {
  ...
  const confirmMsg = {
    bank_first: `Your ${form.role} shift for ${form.date} has been published to your bank staff for 2 hours. If unclaimed, it will automatically escalate to Tier 1 agencies.`,
    agencies:   `Your ${form.role} shift for ${form.date} has been broadcast to all Tier 1 agencies. You'll be notified once a worker is confirmed.`,
    both:       `Your ${form.role} shift for ${form.date} has been sent to both bank staff and Tier 1 agencies simultaneously.`,
  }[form.broadcastTo];
    ...
    <Page title="Request Submitted" icon="✅">
      <div style={{maxWidth:460,background:T.white,borderRadius:16,border:`1px solid ${T.border}`,padding:40,textAlign:"center"}}>
        ...
        <div style={{display:"inline-flex",alignItems:"center",gap:6,padding:"6px 14px",borderRadius:20,background:selectedBroadcast.bg,border:`1px solid ${selectedBroadcast.border}`,marginBottom:14}}>
          <span style={{fontSize:14}}>{selectedBroadcast.icon}</span>
          <span style={{fontSize:12,fontWeight:700,color:selectedBroadcast.color}}>{selectedBroadcast.label}</span>
        </div>
        <p style={{color:T.muted,fontSize:13,lineHeight:1.7,marginBottom:24}}>{confirmMsg}</p>
        <div style={{display:"flex",gap:10,justifyContent:"center"}}>
  ...
  return (
    <Page title="Request a Shift" sub="Submit a new staffing requirement" icon="➕">
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,maxWidth:860,alignItems:"start"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          ...
          <Card style={{padding:24}}>
            ...
            <textarea value={form.notes} onChange={e=>set("notes",e.target.value)} placeholder="Specific requirements, access codes, anything agencies or bank staff need to know…" style={{width:"100%",padding:"10px 12px",border:`1.5px solid ${T.border}`,borderRadius:8,fontSize:13,fontFamily:"Syne,sans-serif",minHeight:90,resize:"vertical",color:T.text,boxSizing:"border-box"}}/>
          </Card>
        </div>

        {/* Right col — broadcast + summary */}
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          <Card style={{padding:24}}>
            <h3 style={{fontWeight:700,fontSize:14,marginBottom:4}}>Publish To</h3>
            <p style={{fontSize:12,color:T.muted,marginBottom:16,lineHeight:1.5}}>Choose who receives this shift first. Bank staff are your internal pool — always cheaper than agency rates.</p>
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {BROADCAST_OPTIONS.map(opt=>{
                const sel=form.broadcastTo===opt.key;
                return(
                  <button key={opt.key} onClick={()=>set("broadcastTo",opt.key)}
                    style={{display:"flex",gap:14,alignItems:"flex-start",padding:"14px 16px",borderRadius:12,border:`2px solid ${sel?opt.color:T.border}`,background:sel?opt.bg:T.white,cursor:"pointer",textAlign:"left",fontFamily:"Syne,sans-serif",transition:"all 0.15s",width:"100%"}}>
                    ...
                    <div style={{flex:1}}>
                      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}>
                        <span style={{fontWeight:700,fontSize:13,color:sel?opt.color:T.text}}>{opt.label}</span>
                        <span style={{fontSize:10,fontWeight:700,padding:"2px 7px",borderRadius:20,background:sel?opt.color+"22":"#f1f5f9",color:sel?opt.tagColor:T.muted}}>{opt.tag}</span>
                      </div>
                      <div style={{fontSize:11,color:T.muted,lineHeight:1.5}}>{opt.desc}</div>
                    </div>
                    <div style={{width:18,height:18,borderRadius:"50%",border:`2px solid ${sel?opt.color:T.border}`,background:sel?opt.color:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:2}}>
                      {sel&&<span style={{color:T.white,fontSize:10,lineHeight:1}}>●</span>}
                    </div>
                  </button>
                );
              })}
          ...
          <Card style={{padding:24}}>
            <div style={{background:T.amberBg,borderRadius:8,padding:"12px 14px",marginBottom:16}}>
              <div style={{fontSize:11,fontWeight:700,color:T.amberText,marginBottom:4}}>Estimated Cost</div>
              <div style={{fontSize:22,fontWeight:800,color:T.amberText}}>
                £{(()=>{const rc=INIT_RATE_CARDS.find(r=>r.type==="client"&&r.role===form.role);const rate=rc?rc.weekday:30;return rate*12;})()}
              </div>
              <div style={{fontSize:11,color:T.amberText,marginTop:2}}>Based on standard rate · Final cost confirmed on fulfilment</div>
            </div>
            {form.broadcastTo==="bank_first"&&(
              <div style={{padding:"10px 12px",borderRadius:8,background:T.tealBg,border:`1px solid #5eead4`,marginBottom:14,fontSize:11,color:T.teal,lineHeight:1.5}}>
                🏦 <strong>Bank staff rate applies</strong> if claimed internally — typically 15–20% lower than agency rates. Agency rate only applies if escalated.
              </div>
            )}
...
const CareHomeInvoices = () => {
  ...
  return (
  <Page title="Invoices" sub="Your billing history from First Choice Connect" icon="📄">
    {viewInv && (
      <Modal title={`Invoice ${viewInv.id}`} onClose={()=>setViewInv(null)}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
...
const AvailableShifts = () => {
  ...
  return (
    <Page title="Available Shifts" sub="Shifts broadcast to First Choice Nursing by First Choice Connect" icon="📋">
      {modal && (
  ...
  if(step>4) return (
    <Page title="Worker Registered" icon="✅">
      <div style={{maxWidth:440,background:T.white,borderRadius:16,border:`1px solid ${T.border}`,padding:40,textAlign:"center"}}>
        ...
        <p style={{color:T.muted,fontSize:13,lineHeight:1.7,marginBottom:24}}><strong>{form.firstName} {form.lastName}</strong> has been added to your worker register. Once documents are verified by First Choice Connect, they'll be available for shift placement.</p>
        <div style={{display:"flex",gap:10,justifyContent:"center"}}>
      ...
      <div style={{maxWidth:620}}>
        ...
        <Card style={{padding:28}}>
          ...
          {step===4 && <>
            ...
            <Alert type="info">After registration, First Choice Connect will review uploaded documents (typically within 1 working day) before activating this worker.</Alert>
          </>}
          <div style={{display:"flex",gap:10,justifyContent:"space-between",marginTop:16}}>
  ...
  <Page title="Compliance Documents" sub="Manage documents for First Choice Nursing workers" icon="📁" action={<Btn onClick={()=>setUploadModal(true)}>Upload Document</Btn>}>
    {uploadModal && (
      <Modal title="Upload Document" onClose={()=>setUploadModal(false)}>
        ...
        <Input label="Expiry Date" type="date" value="" onChange={()=>{}}/>
        <div style={{display:"flex",gap:8}}>
          <Btn onClick={()=>{setUploadModal(false);alert("Document uploaded and sent to FCC for verification.");}}>Upload</Btn>
          <Btn variant="secondary" onClick={()=>setUploadModal(false)}>Cancel</Btn>
        </div>
      </Modal>
    )}
...
const AgencyInvoices = () => {
  ...
  return (
  <Page title="My Invoices" sub="Payment history from First Choice Connect" icon="📄">
    {viewInv && (
      <Modal title={`Invoice ${viewInv.id}`} onClose={()=>setViewInv(null)}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
...
const AdminTimesheets = ({timesheets,setTimesheets,invoices,setInvoices}) => {
  ...
  const tsExports = [
    ...
    {icon:"🖨️",label:"Full Timesheet Report — PDF",fn:()=>exportHTML("Timesheet Report","All timesheets — First Choice Connect",
...
const UsersAndPermissions = ({users,setUsers}) => {
  ...
  return (
    <Page title="Users & Permissions" sub="Manage who has access to First Choice Connect and what they can see and do" icon="🔐"
...
const AgencyUsersAndPermissions = ({users, setUsers, user}) => {
  ...
  return (
    <Page title="Users & Permissions" sub={`Manage who can access the ${myOrg} portal and what they can do`} icon="🔐"
      ...
      <div style={{marginTop:20}}>
        <Card>
          <CardHead title="Default Permission Template" sub="Permissions applied to all new invites. Contact First Choice Connect to request additional section access." icon="📋"/>
          <div style={{padding:"14px 18px"}}>
            <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
              {permDefs.map(p=>(
                <span key={p.k} style={{padding:"4px 10px",borderRadius:20,background:T.greenBg,border:`1.5px solid ${T.green}44`,fontSize:11,fontWeight:600,color:T.green}} title={p.desc}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <Card>
            ...
            <div style={{overflowX:"auto"}}>
              <table style={{width:"100%",borderCollapse:"collapse"}}>
                <thead>
                  <tr style={{background:"#f8fafc"}}>
                    {["Role","Agency Rate","FCC Margin","Client Rate","Margin %"].map(h=>(
          ...
          <Card style={{padding:20}}>
            ...
            <div style={{marginTop:12,padding:"10px 14px",background:T.amberBg,borderRadius:8}}>
              ...
              <div style={{fontSize:12,color:T.amberText}}>If 100 shifts/month are filled (12hrs avg), at these margins FCC earns approx. <strong>£{(sampleRates.reduce((a,r)=>a+getMargin(r.weekday,r.role,cfg)*12,0)/sampleRates.length*100).toFixed(0)}</strong>/month in platform margin.</div>
            </div>
          </Card>

          <Alert type="info">
            <strong>How it works:</strong> Agencies always see and receive their base rate. Care homes are billed the client rate (base + FCC margin). The margin is FCC's platform revenue per hour billed. Changes apply immediately to all new timesheet calculations and invoice generation.
          </Alert>
        </div>
      </div>
    </Page>
                      ...
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        ...
                        <div style={{display:"flex",gap:8}}>
                          <ExportMenu exports={[
                            ...
                            {icon:"🖨️",label:"Export PDF",fn:()=>exportHTML(reportName||"Custom Report","First Choice Connect — Custom Report",buildTable(headers,rows))},
                          ]}/>
  ...
  return (
    <Page title="CQC Readiness Report" sub="One-click compliance summary for inspection readiness" icon="🏅">
      <div style={{background:`linear-gradient(135deg,${T.navy},#1e3a5f)`,borderRadius:16,padding:"24px 28px",marginBottom:20,display:"flex",alignItems:"center",gap:24,flexWrap:"wrap"}}>
        ...
        <Btn onClick={()=>exportHTML("CQC Readiness Report","First Choice Connect","<h2>Generated: "+new Date().toLocaleDateString("en-GB")+"</h2><p>Overall score: "+overallScore+"%</p>")}>Export PDF</Btn>
      </div>
      <Grid cols={4}>
        {sections.map(s=>{
...
const RateUpliftManager = ({user,rateUplifts,setRateUplifts}) => {
  ...
  return (
    <Page title={isAdmin?"Rate Uplift Requests":"Rate Uplift Requests"} sub={isAdmin?"Review and approve agency rate change requests":"Submit a rate change request to First Choice Connect"} icon="📈">
      {isAgency&&(
        <div style={{marginBottom:16}}>
          {showForm?(
            <Card style={{padding:20}}>
...
const RecurringShifts = ({user}) => {
  ...
  return (
    <Page title="Recurring Shift Patterns" sub="Auto-publish shifts on a weekly schedule without manual entry" icon="🔄">
      <Alert type="info">⚡ Active patterns auto-publish shifts every week. FCC will broadcast them to agencies based on your tier configuration.</Alert>
...
const VIEWS = {
  admin:       {
    ...
    documents:DocumentVault,ratecards:RateCards,rateuplifts:RateUpliftManager,
  ...
  clientadmin: {
    ...
    timesheets:CareHomeTimesheets,invoices:CareHomeInvoices,budgets:ClientAdminBudgets,compliance:CareHomeCompliance,
  ...
  carehome:    {
    ...
    myshifts:(p)=><Page title="My Shifts" icon="📋"><Card><Table headers={["Role","Date","Time","Status","Agency","Worker"]} rows={SHIFTS.filter(s=>s.carehome==="Sunrise Care").map(s=><tr key={s.id} style={{borderBottom:`1px solid ${T.border}`}}><Td><Badge label={s.role} color={T.purple} bg={T.purpleBg}/></Td><Td>{s.date}</Td><Td>{s.time}</Td><Td><SBadge s={s.status}/></Td><Td>{s.agency||"—"}</Td><Td>{s.worker||<span style={{color:"#94a3b8"}}>TBC</span>}</Td></tr>)}/></Card></Page>,
